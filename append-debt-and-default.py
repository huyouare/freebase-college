...
with open('test-scores.csv', 'rU') as csvfile:
...
with open('default_rates.csv', 'rU') as csvfile:
  ...
  for row in reader:
    ...
    default_opeid_dict[int(row[0] + '00')] = row # 11 and 23

with open('master-with-sat.csv', 'rU') as csvfile:
  reader = csv.reader(csvfile)
  row = next(reader)
  writer.writerow( row[0:6] + ['Test Scores Required'] + row[6:] + ['Avg Debt of Graduates', 'NBD 1', 'NBR 1'] )
  for row in reader:
    new_row = row
