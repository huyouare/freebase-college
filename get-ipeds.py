...
infile = open('output-fixed.csv', 'rbU')
reader = csv.reader(infile)

with open('output-fixed-with-ipeds.csv', 'wb') as f:
  writer = csv.writer(f)
  ...
  for row in reader:
    ...
    if freebase_id and freebase_id != "":
      ...
      if 'result' not in response.keys():
        ...
        writer.writerow([row[0], row[1], ipeds, row[2], row[3], row[4], row[5], row[6], row[7], row[8]])
